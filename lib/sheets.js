...
async function ensureMonthlySheet(sheetsApi, spreadsheetId, sheetName) {
  ...
  if (sheetId !== null) {
    ...
    const headerCheck = await withRetry(() => sheetsApi.spreadsheets.values.get({ spreadsheetId, range: `${sheetName}!B2` }));
  ...
  // Flat dot-paths, not the grouped "userEnteredFormat(a,b,c)" shorthand —
  // that grouped form is valid per Google's docs, but in this deployment it
  // silently failed to apply (background/alignment never took effect) while
  // the flat form used for numberFormat and wrapStrategy elsewhere in this
  // file demonstrably works. Matching the proven pattern rather than the
  // documented-but-unreliable one.
  const headerFields = ["userEnteredFormat.backgroundColor", "userEnteredFormat.textFormat.bold", "userEnteredFormat.textFormat.foregroundColor", "userEnteredFormat.horizontalAlignment", "userEnteredFormat.verticalAlignment"].join(",");

  // Data rows: wrapped and centred both ways.
  const dataCell = {
    userEnteredFormat: {
      wrapStrategy: "WRAP",
      horizontalAlignment: "CENTER",
  ...
  const dataFields = ["userEnteredFormat.wrapStrategy", "userEnteredFormat.horizontalAlignment", "userEnteredFormat.verticalAlignment"].join(",");

  // Thin black grid on all four sides of every cell in a block.
  const borderLine = { style: "SOLID", width: 1, color: black };
  const borderedCell = {
    userEnteredFormat: {
      borders: { top: borderLine, bottom: borderLine, left: borderLine, right: borderLine },
    },
  };
  const borderFields = "userEnteredFormat.borders";
      ...
      sheetsApi.spreadsheets.batchUpdate({
        ...
        requestBody: {
          requests: [
            ...
            // Notifikasi columns need extra room
            ...
            // Every data cell in all four blocks: wrapped and centred both
            // ways, so a long Notifikasi doesn't spill and short values don't
            // sit flush against a cell edge.
            { repeatCell: { range: grid(3, DATA_CAP_ROW, 1, 5), cell: dataCell, fields: dataFields } }, // B4:E
            { repeatCell: { range: grid(3, DATA_CAP_ROW, 6, 10), cell: dataCell, fields: dataFields } }, // G4:J
            { repeatCell: { range: grid(3, DATA_CAP_ROW, 11, 14), cell: dataCell, fields: dataFields } }, // L4:N
            { repeatCell: { range: grid(3, 4, 15, 17), cell: dataCell, fields: dataFields } }, // P4:Q4

            // Cell borders across each block (headers included) so the sheet
            // reads as a table rather than loose values on a grid.
            { repeatCell: { range: grid(1, DATA_CAP_ROW, 1, 5), cell: borderedCell, fields: borderFields } }, // B2:E
            { repeatCell: { range: grid(1, DATA_CAP_ROW, 6, 10), cell: borderedCell, fields: borderFields } }, // G2:J
            { repeatCell: { range: grid(1, DATA_CAP_ROW, 11, 14), cell: borderedCell, fields: borderFields } }, // L2:N
            { repeatCell: { range: grid(1, 4, 15, 17), cell: borderedCell, fields: borderFields } }, // P2:Q4
...
/**
 * Finds which row a new transaction dated `when` should occupy so the block
 * stays ordered by date/time top to bottom, and makes room for it there.
 *
 * Existing rows are read as-is (gaps from deleted rows are skipped, not
 * treated as ordering boundaries). If the new date is the newest so far it's
 * placed right after the last occupied row. Otherwise it's slotted in front
 * of the first row that's later than it: reusing a gap immediately before
 * that row if one exists, or shifting that row and everything after it down
 * by one (within this block's own columns only) to make space.
 */
async function findInsertionRow(sheetsApi, spreadsheetId, sheetName, block, when) {
  const [startCol, endCol] = block.cols.split(":");
  const res = await withRetry(() =>
    sheetsApi.spreadsheets.values.get({
      spreadsheetId,
      range: `${sheetName}!${startCol}${block.startRow}:${endCol}${DATA_CAP_ROW}`,
  ...
  if (lastDataIndex === -1) return block.startRow;

  const occupied = [];
  for (let i = 0; i <= lastDataIndex; i++) {
    const cell = rows[i] && rows[i][0];
    if (cell === undefined || cell === "") continue;
    const parsed = parseDateTimeWIB(cell);
    occupied.push({ index: i, row: block.startRow + i, time: parsed ? parsed.getTime() : 0 });
  }

  const newTime = when.getTime();
  let insertBeforeIdx = occupied.length;
  for (let k = 0; k < occupied.length; k++) {
    if (occupied[k].time > newTime) {
      insertBeforeIdx = k;
      break;
    }
  }

  if (insertBeforeIdx === occupied.length) {
    return occupied.length ? occupied[occupied.length - 1].row + 1 : block.startRow;
  }

  const targetRow = occupied[insertBeforeIdx].row;
  const prevRow = insertBeforeIdx > 0 ? occupied[insertBeforeIdx - 1].row : block.startRow - 1;

  if (targetRow - prevRow > 1) {
    // A gap left by an earlier delete sits right before this row — reuse it.
    return targetRow - 1;
  }

  // No room: push this row and everything after it down by one first.
  const shiftValues = rows.slice(occupied[insertBeforeIdx].index, lastDataIndex + 1);
  await withRetry(() =>
    sheetsApi.spreadsheets.values.update({
      spreadsheetId,
      range: `${sheetName}!${startCol}${targetRow + 1}:${endCol}${targetRow + shiftValues.length}`,
      valueInputOption: "RAW",
      requestBody: { values: shiftValues },
    }),
  );
  return targetRow;
}
...
async function listTransactionsForSheet(sheetsApi, spreadsheetId, sheetName) {
  ...
  try {
    res = await withRetry(() =>
      sheetsApi.spreadsheets.values.batchGet({
        spreadsheetId,
        ranges: [`${sheetName}!B${BLOCKS.expense.startRow}:E${DATA_CAP_ROW}`, `${sheetName}!G${BLOCKS.income.startRow}:J${DATA_CAP_ROW}`],
        valueRenderOption: "UNFORMATTED_VALUE",
      }),
    );
  } catch (err) {
  ...
  return [...readBlockRows(expenseRange, sheetName, "expense", BLOCKS.expense.startRow), ...readBlockRows(incomeRange, sheetName, "income", BLOCKS.income.startRow)];
}
...
function computeRecap(transactions) {
  ...
  const daily = [...byDay.entries()].sort((a, b) => a[1].sortKey - b[1].sortKey).map(([day, v]) => [day, v.expense, v.income]);
...
async function appendTransaction({ type, amount, source, raw, date }) {
  ...
  const row = await findInsertionRow(sheetsApi, spreadsheetId, sheetName, block, when);
...
async function listCycleSheets() {
  ...
  const meta = await withRetry(() => sheetsApi.spreadsheets.get({ spreadsheetId, fields: "sheets(properties(title))" }));
...
async function updateTransaction(id, updates) {
  ...
  const merged = [tanggal, updates.amount !== undefined ? updates.amount : parseAmount(jumlah), updates.source !== undefined ? updates.source : sumber, updates.raw !== undefined ? updates.raw : notifikasi];
...
async function getRecapMarker(kind, sheetName) {
  ...
  const spreadsheetId = getSpreadsheetId();
  try {
    const res = await withRetry(() => sheetsApi.spreadsheets.values.get({ spreadsheetId, range: `${sheetName}!${MARKER_CELLS[kind]}` }));
