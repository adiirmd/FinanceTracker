...
/**
 * Sends a Telegram alert for every Rp50.000 boundary today's spending crosses
 * because of this transaction.
 *
 * Backdated entries never alert at all. This is a real-time budget warning
 * about what's being spent *today*; an entry recorded for a past date is
 * bookkeeping (a missed transaction, or demo data), and firing "pengeluaran
 * hari ini sudah mencapai..." for it would be both wrong and noisy. A single
 * large historical entry would spam a run of threshold messages about a day
 * that's already over.
 *
...
async function checkSpendingAlert(justAdded) {
  ...
  const dayStart = startOfTodayWIB().getTime();
  const dayEnd = dayStart + DAY_MS;

  // Not today's spending -> not this alert's business.
  const txMs = txDate.getTime();
  if (txMs < dayStart || txMs >= dayEnd) {
    return { alerted: false, reason: "backdated" };
  }

  const all = await listTransactions({ cycle: cycleSheetName(txDate) });
